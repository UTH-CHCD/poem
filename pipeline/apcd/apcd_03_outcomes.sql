...
SELECT DISTINCT 
    c.apcd_id,
    c.ep_num,
    CASE WHEN d90.apcd_id IS NOT NULL THEN 1 ELSE 0 END AS out_enroll_90,
    CASE WHEN d12.apcd_id IS NOT NULL THEN 1 ELSE 0 END AS out_enroll_12
FROM research_dev.poem_cohort c
LEFT JOIN research_di.agg_yrmon_plan AS d90
    ON d90.apcd_id = c.apcd_id
...
with_next AS (
    SELECT 
        ...
        anchor_date,
        months_from_anchor,
        LEAD(months_from_anchor) OVER (PARTITION BY apcd_id, ep_num ORDER BY months_from_anchor) AS next_month
...
SELECT 
    ...
    END AS ce_after
FROM gap_detection
GROUP BY apcd_id, ep_num, anchor_date;
...
WITH enrollment_months AS (
    ...
    FROM research_dev.poem_cohort c
    INNER JOIN research_di.agg_enrl_yrmon d
        ON d.apcd_id = c.apcd_id
       AND d.yrmon < CAST(TO_CHAR(DATE_TRUNC('month', c.anchor_date), 'YYYYMM') AS INTEGER)
),
with_next AS (
    SELECT 
        ...
        LEAD(months_before_anchor) OVER (PARTITION BY apcd_id, ep_num ORDER BY months_before_anchor DESC) AS next_month
    FROM enrollment_months
),
gap_detection AS (
    SELECT 
...
LEFT JOIN research_di.agg_yrmon_plan d
    ...
    and d.prim_med_plan = 'Medicaid'
   AND d.yrmon < CAST(TO_CHAR(DATE_TRUNC('month', c.anchor_date), 'YYYYMM') AS INTEGER)
GROUP BY c.apcd_id, c.ep_num, c.anchor_date;
...
LEFT JOIN research_dev.poem_outcomes_enrollment e
    ON e.apcd_id = c.apcd_id AND e.ep_num = c.ep_num
LEFT JOIN research_dev.poem_outcomes_ce_after ca
    ON ca.apcd_id = c.apcd_id AND ca.ep_num = c.ep_num
LEFT JOIN research_dev.poem_outcomes_ce_before cb
    ON cb.apcd_id = c.apcd_id AND cb.ep_num = c.ep_num
LEFT JOIN research_dev.poem_outcomes_total_after ta
    ON ta.apcd_id = c.apcd_id AND ta.ep_num = c.ep_num
LEFT JOIN research_dev.poem_outcomes_total_before tb
    ON tb.apcd_id = c.apcd_id AND tb.ep_num = c.ep_num;
...
SELECT
    ...
    c.ep_num,
    p.code_type,
    CASE WHEN COUNT(p.apcd_id) > 0 THEN 1 ELSE 0 END AS out_postpartum,
...
SELECT
    ...
    c.ep_num,
    p.code_type,
    CASE WHEN COUNT(p.apcd_id) > 0 THEN 1 ELSE 0 END AS out_postpartum_6,
...
SELECT
    ...
    c.ep_num,
    p.code_type,
    CASE WHEN COUNT(p.apcd_id) > 0 THEN 1 ELSE 0 END AS out_postpartum_12,
...
LEFT JOIN research_dev.poem_outcomes_out_postpartum6 b
    ON a.apcd_id = b.apcd_id AND a.ep_num = b.ep_num
LEFT JOIN research_dev.poem_outcomes_out_postpartum12 c
    ON a.apcd_id = c.apcd_id AND a.ep_num = c.ep_num
GROUP BY a.apcd_id, a.ep_num;
...
JOIN research_dev.test_type_lookup t
    ON p.proc_cd = t.code
   where p.apcd_id in (select apcd_id from research_dev.poem_cohort);
...
SELECT
    ...
    c.ep_num,
    p.test_type,
    CASE WHEN COUNT(p.apcd_id) > 0 THEN 1 ELSE 0 END AS out_diab_screen_6,
    COUNT(DISTINCT p.code) AS code_count_6
FROM research_dev.poem_cohort c
...
SELECT
    ...
    c.ep_num,
    p.test_type,
    CASE WHEN COUNT(p.apcd_id) > 0 THEN 1 ELSE 0 END AS out_diab_screen_12,
    COUNT(DISTINCT p.code) AS code_count_12
FROM research_dev.poem_cohort c
LEFT JOIN research_dev.poem_diab_screen_temp p
    ON c.apcd_id = p.apcd_id
...
WITH DistinctCombos AS (
    SELECT DISTINCT
        ...
        test_type
    FROM (
        SELECT apcd_id, ep_num, test_type FROM research_dev.poem_outcomes_out_diab_screen6
        UNION
        SELECT apcd_id, ep_num, test_type FROM research_dev.poem_outcomes_out_diab_screen12
    ) AS Combined
)
SELECT
    d.apcd_id,
...
SELECT
    ...
    c.ep_num,
    p.htn_category,
    CASE WHEN COUNT(p.apcd_id) > 0 THEN 1 ELSE 0 END AS out_htn_med_6,
...
SELECT
    ...
    c.ep_num,
    p.htn_category,
    CASE WHEN COUNT(p.apcd_id) > 0 THEN 1 ELSE 0 END AS out_htn_med_12,
...
WITH DistinctCombos AS (
    SELECT DISTINCT
        ...
        htn_category
    FROM (
        SELECT apcd_id, ep_num, htn_category FROM research_dev.poem_outcomes_out_htn_med6
        UNION
        SELECT apcd_id, ep_num, htn_category FROM research_dev.poem_outcomes_out_htn_med12
    ) AS Combined
)
SELECT
    d.apcd_id,
...
WITH get_all AS (
    SELECT a.apcd_id, a.derv_pccn AS clm_id, dos_thru AS to_dos
    ...
    UNION ALL
    SELECT a.apcd_id, a.derv_pccn AS clm_id, dos AS to_dos
...
SELECT apcd_id, 
       ...
       MIN(to_dos) AS date_
FROM get_all 
where apcd_id in (select apcd_id from research_dev.poem_cohort )
GROUP BY apcd_id, clm_id;
...
SELECT DISTINCT apcd_id, derv_pccn AS clm_id, 'postpartum_evaluation' AS visit_type
...
FROM (
    SELECT apcd_id, derv_pccn AS clm_id  
    ...
    UNION ALL
    SELECT DISTINCT apcd_id, derv_pccn AS clm_id
    FROM research_dev.med_dx_poem 
    WHERE dx IN ('Z0000', 'Z0001', 'Z00121', 'Z00129')
    and apcd_id in (select apcd_id from research_dev.poem_cohort )
) a;
...
SELECT DISTINCT apcd_id, derv_pccn AS clm_id, 'contraceptive' AS visit_type
FROM research_dev.med_dx_poem 
...
FROM (
    ...
    UNION ALL
    SELECT DISTINCT c.apcd_id, c.derv_pccn AS clm_id
    FROM research_dev.medical_adj_poem c
    JOIN research_dev.med_dx_poem d
        ON c.apcd_id = d.apcd_id 
        AND c.derv_pccn = d.derv_pccn
    WHERE c.proc_cd IN (
...
SELECT a.*, 
       CASE WHEN b.apcd_id IS NOT NULL THEN 1 ELSE 0 END AS pe,
       CASE WHEN c.apcd_id IS NOT NULL THEN 1 ELSE 0 END AS prev,
       CASE WHEN d.apcd_id IS NOT NULL THEN 1 ELSE 0 END AS contr,
       CASE WHEN e.apcd_id IS NOT NULL THEN 1 ELSE 0 END AS mental,
       CASE WHEN b.apcd_id IS NULL 
            AND c.apcd_id IS NULL 
            AND d.apcd_id IS NULL 
            AND e.apcd_id IS NULL THEN 1 ELSE 0 END AS other
...
SELECT
    ...
    CASE WHEN COUNT(op.apcd_id) > 0 THEN 1 ELSE 0 END AS out_any_outpatient_12,

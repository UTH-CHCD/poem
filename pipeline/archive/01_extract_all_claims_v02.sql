...
with ranked_records as (
    ...
    group by client_nbr, outcome_type, date_trunc('month', clm_from_date)
),
streaks as (
    select
        client_nbr,
...
vacuum analyze dev.poem_all_dx_streaks;
...
----------------------------------------------------
 ...
 drop table if exists dev.poem_all_dx_streaks_contradictions;
 
select * 
 into dev.poem_all_dx_streaks_contradictions
 from dev.poem_all_dx_streaks a
where exists (
    select 1
    from dev.poem_all_dx_streaks b
...
delete from dev.poem_all_dx_streaks a
where exists (
    select 1
    from dev.poem_all_dx_streaks b
    where a.client_nbr = b.client_nbr
...
select count(distinct streak_id::text || client_nbr) episodes 
  from dev.poem_all_dx_streaks_contradictions ;
--4108

select count(distinct streak_id::text || client_nbr) episodes 
  from dev.poem_all_dx_streaks_contradictions 
  where streak_id::text || client_nbr 
    not in (select distinct streak_id::text || client_nbr from dev.poem_all_dx_streaks) ;
--1113
*/
...
drop table if exists dev.poem_all_dx_streaks_quarantine ;   
   
select * 
  into dev.poem_all_dx_streaks_quarantine 
  from dev.poem_all_dx_streaks 
...
select distinct a.* , b.dob, 
 ...
 --- Filter table by age
drop table if exists dev.poem_episodes_age;

create table dev.poem_episodes_age as 
select * 
  from dev.poem_episodes_enrollment where age between 15 and 55;
...
--- Create the temporary table with the combined data from medicaid.enrl and medicaid.chip_enrl
 -- Take enrl over chip_enrl, since it is more likely to be more recent 
 -- (people go from chip -> medicaid, less likely the reverse)
drop table if exists dev.poem_temp_zips;

create table dev.poem_temp_zips as
select distinct client_nbr,
                to_date(elig_date, 'YYYYMM') as elig_month,
                substring(zip, 1, 5) as zip
   from medicaid.enrl
  where extract(year from to_date(elig_date, 'YYYYMM')) > 2018
    and sex = 'F'
union all
 select distinct chip.client_nbr,
                to_date(chip.elig_month, 'YYYYMM') as elig_month,
                substring(chip.mailing_zip, 1, 5) as zip
    from medicaid.chip_enrl chip
    left join medicaid.enrl enrl
      on chip.client_nbr = enrl.client_nbr
     and to_date(chip.elig_month, 'YYYYMM') = to_date(enrl.elig_date, 'YYYYMM')
    where enrl.client_nbr is null
      and gender_cd = 'F'
      and extract(year from to_date(chip.elig_month, 'YYYYMM')) > 2018;  
     
-- make sure no duplicate months 
/*
 select * 
   from dev.poem_temp_zips where (client_nbr, elig_month) in ( 
  select client_nbr , elig_month
   from dev.poem_temp_zips
  group by client_nbr , elig_month having count(*) > 1
 ) order by client_nbr, elig_month ;    
 */
...
drop table if exists dev.poem_episodes_age_tx;
 
 create table dev.poem_episodes_age_tx as 
 select distinct a.*, b.zip
   from dev.poem_episodes_age a 
   left join dev.poem_temp_zips_tx b 
     on a.client_nbr = b.client_nbr
    and to_char(clm_from_date, 'YYYYMM') = to_char(elig_month, 'YYYYMM');
   
  select count(*) 
  from dev.poem_episodes_age_tx ;
  
 
 select client_nbr , elig_month , count(*)
   from dev.poem_temp_zips_tx 
  group by client_nbr , elig_month having count(*) > 1;

...
-- Insert just the claims with admit and discharge  
 select distinct *
   ...
   where admit_date is not null and discharge_date is not null and pat_stat = '01' 
  order by client_nbr;
...
-- Fix Dates
UPDATE chcdwork.dbo.poem_all_dx_simplify
   SET admit_date = DATEFROMPARTS(DATEPART(year, clm_from_date), DATEPART(month, admit_date), DATEPART(day, admit_date))
 WHERE DATEPART(month, admit_date) = DATEPART(month, clm_from_date)
   AND DATEPART(day, admit_date) = DATEPART(day, clm_from_date)
   AND DATEPART(year, admit_date) <> DATEPART(year, clm_from_date);


UPDATE chcdwork.dbo.poem_all_dx_simplify
SET discharge_date = DATEFROMPARTS(DATEPART(year, clm_to_date), DATEPART(month, discharge_date), DATEPART(day, discharge_date))
WHERE DATEPART(month, discharge_date) = DATEPART(month, clm_to_date)
  AND DATEPART(day, discharge_date) = DATEPART(day, clm_to_date)
  AND DATEPART(year, discharge_date) <> DATEPART(year, clm_to_date);

UPDATE chcdwork.dbo.poem_all_dx_simplify
   SET discharge_date = NULL
 WHERE DATEPART(year, discharge_date) < 2018;

UPDATE chcdwork.dbo.poem_all_dx_simplify
   SET admit_date = NULL
 WHERE DATEPART(year, admit_date) < 2018;

UPDATE chcdwork.dbo.poem_all_dx_simplify
   SET admit_date = NULL 
 where admit_date > discharge_date 
   or  admit_date > clm_to_date;

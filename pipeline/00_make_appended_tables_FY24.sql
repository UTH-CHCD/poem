...
union all 
select * from MEDICAID.dbo.CLM_DX_24
) a ;
...
union all 
select * from MEDICAID.dbo.CLM_HEADER_24
) a;
...
union all 
select * from MEDICAID.dbo.CLM_PROC_24
) a;
...
union all 
select derv_enc, adm_dt, dis_dt, frm_dos, to_dos, pat_stat from MEDICAID.dbo.ENC_HEADER_24
) a;
...
union all 
select * from MEDICAID.dbo.ENRL_2024
) a
where sex = 'F';
...
union all 
select * from MEDICAID.dbo.CHIP_ENRL_FY24
) a 
where gender_cd = 'F';

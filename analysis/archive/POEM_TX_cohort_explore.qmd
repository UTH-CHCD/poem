...
# Dates associated with Pregnancy

Combining all possible dates together (admit, discharge, claim_from_date, claim_to_date) to see the distribution of dates for a single episode, we see:

```{r}
dates.per.episode <- dbGetQuery(spc, " with get_count as (
 select client_nbr , ep_num , count(distinct date_) as c
  from CHCDWORK.dbo.poem_episodes_alldates
  group by client_nbr , ep_num 
  )
  select c as unique_dates_per_episode, count(distinct client_nbr + cast(ep_num as varchar)) as episode_count
    from get_count
    group by c
    order by c;
    ")

dates.per.episode %>% flextable() %>% 
  theme_box() %>%
  padding(padding = 1.5) %>%
  autofit() 

# ggplot(dates.per.episode, aes(x=unique_dates_per_episode, y=episode_count)) +

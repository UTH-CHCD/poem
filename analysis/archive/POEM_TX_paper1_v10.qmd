...
# Updated time periods with Unwinding period added
time_periods.6months <- tibble::tibble(
  period = factor(c("pre_FFCRA", "COVID", "FFCRA", "Unwinding"), 
                  levels = c("pre_FFCRA", "COVID", "FFCRA", "Unwinding")),
  start = as.Date(c("2019-01-01", "2019-09-19", "2020-04-18", "2022-10-01")),
  end   = as.Date(c("2019-09-18", "2020-04-17", "2022-09-30", "2023-08-31"))
)
...
time_periods.6months.old <- tibble::tibble(
  period = factor(c("pre_FFCRA", "COVID", "FFCRA", "Unwinding"), 
                  levels = c("pre_FFCRA", "COVID", "FFCRA", "Unwinding")),
  start = as.Date(c("2019-01-01", "2019-09-19", "2020-07-01", "2022-10-01")),
  end   = as.Date(c("2019-09-18", "2020-06-30", "2022-09-30", "2023-08-31"))
)



# Get a date six months before end of FY23
six_months_before <- as.Date("2024-08-31") %m-% months(6)
...
table_by_group <- long_subcohort %>%
  ...
  select(group, outcome_type, anchor_year, race_desc, plan_, dual, age_group, hispanic, smm_weight, no_transfusion_weight) %>%
  tbl_summary(by = group)

# Overall
table_overall <- sub.cohort %>%
  select(outcome_type, anchor_year, race_desc, plan_, dual, age_group, hispanic, smm_weight, no_transfusion_weight) %>%
  tbl_summary()
...
```{r}
# diab.subgroup %>%
#   filter(period_COVID + period_FFCRA + period_Unwinding == 0) %>% nrow()


# diab.subgroup %>% 
#   filter(period_Unwinding == 1) %>%
#   summarise(max = max(anchor_date), min = min(anchor_date))

# diab.subgroup %>%
#   filter(is.na(period_Unwinding))
...
models <- list(
  # Diabetes Screening
  diab_screen_unadj = lm(out_diab_screen_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
                           biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding,
                         data = diab.subgroup),

  diab_screen_adj   = lm(out_diab_screen_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
                           biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding +
                           age + no_transfusion_weight,
                         data = diab.subgroup),

  # Diabetes Postpartum Visits
  diab_postpartum_unadj = lm(out_postpartum_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
                               biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding,
                             data = diab.subgroup),

  diab_postpartum_adj   = lm(out_postpartum_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
                               biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding +
                               age + no_transfusion_weight,
                             data = diab.subgroup),

  # HTN Medication
  htn_med_unadj = lm(out_htn_med_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
                       biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding,
                     data = htn.subgroup),

  htn_med_adj   = lm(out_htn_med_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
                       biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding +
                       age + no_transfusion_weight,
                     data = htn.subgroup),

  # HTN Postpartum Visits
  htn_postpartum_unadj = lm(out_postpartum_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
                              biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding,
                            data = htn.subgroup),

  htn_postpartum_adj   = lm(out_postpartum_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
                              biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding +
                              age + no_transfusion_weight,
                            data = htn.subgroup)
)
...
# alternative time-period flags (same logic, just compact)
cohort_alt <- cohort %>%
  dplyr::filter(anchor_date <= twelve_months_before) %>%
  dplyr::mutate(
    period_COVID     = as.integer(dplyr::between(
      anchor_date,
      dplyr::filter(time_periods.6months.old, period == "COVID")$start,
      dplyr::filter(time_periods.6months.old, period == "COVID")$end
    )),
    period_FFCRA     = as.integer(dplyr::between(
      anchor_date,
      dplyr::filter(time_periods.6months.old, period == "FFCRA")$start,
      dplyr::filter(time_periods.6months.old, period == "FFCRA")$end
    )),
    period_Unwinding = as.integer(dplyr::between(
      anchor_date,
      dplyr::filter(time_periods.6months.old, period == "Unwinding")$start,
      dplyr::filter(time_periods.6months.old, period == "Unwinding")$end
    ))
  )

# Alt subgroups
diab.subgroup_alt <- dplyr::filter(cohort_alt, diab_sample == 1)
htn.subgroup_alt  <- dplyr::filter(cohort_alt,  htn_sample  == 1)

# Alt models
models$diab_screen_unadj_alt <- lm(
  out_diab_screen_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
    biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding,
  data = diab.subgroup_alt
)
models$diab_screen_adj_alt <- lm(
  out_diab_screen_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
    biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding +
    age + no_transfusion_weight,
  data = diab.subgroup_alt
...
models$diab_postpartum_unadj_alt <- lm(
  out_postpartum_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
    biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding,
  data = diab.subgroup_alt
)
models$diab_postpartum_adj_alt <- lm(
  out_postpartum_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
    biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding +
    age + no_transfusion_weight,
  data = diab.subgroup_alt
...
models$htn_med_unadj_alt <- lm(
  out_htn_med_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
    biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding,
  data = htn.subgroup_alt
)
models$htn_med_adj_alt <- lm(
  out_htn_med_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
    biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding +
    age + no_transfusion_weight,
  data = htn.subgroup_alt
...
models$htn_postpartum_unadj_alt <- lm(
  out_postpartum_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
    biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding,
  data = htn.subgroup_alt
)
models$htn_postpartum_adj_alt <- lm(
  out_postpartum_12 ~ biweekly_counter + period_COVID + period_FFCRA + period_Unwinding +
    biweekly_counter:period_COVID + biweekly_counter:period_FFCRA + biweekly_counter:period_Unwinding +
    age + no_transfusion_weight,
  data = htn.subgroup_alt
...
```{r}
compare_models(
  c("diab_screen_unadj", "diab_screen_unadj_alt",
    "diab_screen_adj",   "diab_screen_adj_alt"),
  c("Original - Unadj",  "Alternative - Unadj",
...
```{r}
compare_models(
  c("diab_postpartum_unadj", "diab_postpartum_unadj_alt",
    "diab_postpartum_adj",   "diab_postpartum_adj_alt"),
  c("Original - Unadj",      "Alternative - Unadj",
    "Original - Adj",        "Alternative - Adj")
) |>
  as_flex_table() |>
...
```{r}
compare_models(
  c("htn_med_unadj", "htn_med_unadj_alt",
    "htn_med_adj",   "htn_med_adj_alt"),
  c("Original - Unadj", "Alternative - Unadj",
    "Original - Adj",   "Alternative - Adj")
) |>
  as_flex_table() |>
...
```{r}
compare_models(
  c("htn_postpartum_unadj", "htn_postpartum_unadj_alt",
    "htn_postpartum_adj",   "htn_postpartum_adj_alt"),
  c("Original - Unadj",     "Alternative - Unadj",
    "Original - Adj",       "Alternative - Adj")
) |>
  as_flex_table() |>
...
```{r}
# 1) Calendar points for compression
origin      <- as.Date("2019-01-01")
covid_start <- time_periods.6months %>% dplyr::filter(period == "COVID") %>% dplyr::pull(start)
covid_end   <- time_periods.6months %>% dplyr::filter(period == "COVID") %>% dplyr::pull(end)
covid_days  <- as.integer(covid_end - covid_start + 1L)

# 2) Build no-COVID datasets and RECOMPUTE biweekly_counter (same variable name)
diab.subgroup_nocovid <- diab.subgroup %>%
  dplyr::filter(period_COVID == 0) %>%
  dplyr::mutate(
    # compress days after the COVID window so the time index is contiguous
    adj_days = dplyr::if_else(anchor_date > covid_end, covid_days, 0L),
    biweekly_counter = as.integer((as.integer(anchor_date - origin) - adj_days) / 14) + 1L,
    anchor_date_twoweeks = origin + (biweekly_counter - 1L) * 14L
  ) %>%
  dplyr::select(-adj_days)

htn.subgroup_nocovid <- htn.subgroup %>%
  dplyr::filter(period_COVID == 0) %>%
  dplyr::mutate(
    adj_days = dplyr::if_else(anchor_date > covid_end, covid_days, 0L),
    biweekly_counter = as.integer((as.integer(anchor_date - origin) - adj_days) / 14) + 1L,
...
# 3) Build RHS from your stored models and drop ANY term involving period_COVID
drop_covid_rhs <- function(model_obj) {
  tl <- attr(terms(model_obj), "term.labels")
  keep <- !grepl("\\bperiod_COVID\\b", tl)   # drop main + any interactions containing period_COVID
  paste(tl[keep], collapse = " + ")
}
rhs_unadj_nc <- drop_covid_rhs(models$diab_screen_unadj)
rhs_adj_nc   <- drop_covid_rhs(models$diab_screen_adj)

fit_nocovid_reindexed <- function(base_label, outcome, data, adjusted = FALSE) {
  rhs <- if (adjusted) rhs_adj_nc else rhs_unadj_nc
  fml <- as.formula(paste(outcome, "~", rhs))
  nm  <- paste0(base_label, if (adjusted) "_adj_nocovid_reidx" else "_unadj_nocovid_reidx")
  models[[nm]] <<- lm(fml, data = data)
  nm
}

# 4) Fit the 4 outcomes × (unadj/adj) using the reindexed counter (name unchanged)
# Diabetes cohort
fit_nocovid_reindexed("diab_screen",     "out_diab_screen_12", diab.subgroup_nocovid, adjusted = FALSE)
fit_nocovid_reindexed("diab_screen",     "out_diab_screen_12", diab.subgroup_nocovid, adjusted = TRUE)
fit_nocovid_reindexed("diab_postpartum", "out_postpartum_12",  diab.subgroup_nocovid, adjusted = FALSE)
fit_nocovid_reindexed("diab_postpartum", "out_postpartum_12",  diab.subgroup_nocovid, adjusted = TRUE)

# HTN cohort
fit_nocovid_reindexed("htn_med",         "out_htn_med_12",     htn.subgroup_nocovid,  adjusted = FALSE)
fit_nocovid_reindexed("htn_med",         "out_htn_med_12",     htn.subgroup_nocovid,  adjusted = TRUE)
fit_nocovid_reindexed("htn_postpartum",  "out_postpartum_12",  htn.subgroup_nocovid,  adjusted = FALSE)
fit_nocovid_reindexed("htn_postpartum",  "out_postpartum_12",  htn.subgroup_nocovid,  adjusted = TRUE)
...
compare_models(
  c("diab_screen_unadj", "diab_screen_unadj_nocovid_reidx",
    "diab_screen_adj",   "diab_screen_adj_nocovid_reidx"),
  c("Original - Unadj",  "No-COVID (reindexed) - Unadj",
    "Original - Adj",    "No-COVID (reindexed) - Adj")
...
```{r}
compare_models(
  c("diab_postpartum_unadj", "diab_postpartum_unadj_nocovid_reidx",
    "diab_postpartum_adj",   "diab_postpartum_adj_nocovid_reidx"),
  c("Original - Unadj",      "No-COVID (reindexed) - Unadj",
    "Original - Adj",        "No-COVID (reindexed) - Adj")
) |> as_flex_table() |> FitFlextableToPage()
```
...
```{r}
compare_models(
  c("htn_med_unadj", "htn_med_unadj_nocovid_reidx",
    "htn_med_adj",   "htn_med_adj_nocovid_reidx"),
  c("Original - Unadj", "No-COVID (reindexed) - Unadj",
    "Original - Adj",   "No-COVID (reindexed) - Adj")
) |> as_flex_table() |> FitFlextableToPage()
```
...
```{r}
compare_models(
  c("htn_postpartum_unadj", "htn_postpartum_unadj_nocovid_reidx",
    "htn_postpartum_adj",   "htn_postpartum_adj_nocovid_reidx"),
  c("Original - Unadj",     "No-COVID (reindexed) - Unadj",
    "Original - Adj",       "No-COVID (reindexed) - Adj")
) |> as_flex_table() |> FitFlextableToPage()

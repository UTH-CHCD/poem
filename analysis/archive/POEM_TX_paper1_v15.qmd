...
cohort %>%
  group_by(anchor_date_twoweeks) %>%
  summarise(pct_mh = mean(mh_sample)) %>%
  ungroup() %>%
  ggplot(aes(x = anchor_date_twoweeks, y = pct_mh)) +
  geom_line(linewidth = 1) +
  scale_y_continuous(labels = percent, limits = c(0, 1)) +
  scale_color_brewer(palette = "Set2") +
  labs(
    title = "% of cohort with mental health claims on anchor date or within period of 280 days prior",
    x = "Date",
    y = "% with mental health DX codes",
    color = "Period Start"
  ) +
  theme_minimal() +
  theme(

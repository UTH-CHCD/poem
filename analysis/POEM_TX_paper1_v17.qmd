...
time_periods <- tibble::tibble(
  period = factor(c("Pre_period", "FFCRA", "Unwinding", "Exposure_Unwinding"), 
                  levels = c("Pre_period", "FFCRA", "Unwinding", "Exposure_Unwinding")),
  start = as.Date(c("2019-01-01", "2020-01-01", "2023-04-01", "2022-04-01")),
  end   = as.Date(c("2019-12-31", "2022-03-31", "2023-08-31", "2023-08-31"))  # Changed FFCRA end date
)
...
# Get a date six months before end of FY24
...
cohort <- cohort %>%
  mutate(
    ...
    period_Exposure_Unwinding = case_when(
      anchor_date >= time_periods %>% filter(period == "Exposure_Unwinding") %>% pull(start) &
      anchor_date <= time_periods %>% filter(period == "Exposure_Unwinding") %>% pull(end) ~ 1,
      TRUE ~ 0
    ),
...
# Check period sums to make sure no overlapping periods
cohort_check <- cohort %>%
  mutate(
    period_sum = period_Pre + period_FFCRA + period_Exposure_Unwinding
  )
...
models <- list(
  # Diabetes Screening
  diab_screen_unadj = lm(out_diab_screen_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                           biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding,
                         data = diab.subgroup),
  diab_screen_adj   = lm(out_diab_screen_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                           biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding +
                           age + no_transfusion_weight,
                         data = diab.subgroup),
  
  # Diabetes Postpartum Visits
  diab_postpartum_unadj = lm(out_postpartum_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                               biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding,
                             data = diab.subgroup),
  diab_postpartum_adj   = lm(out_postpartum_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                               biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding +
                               age + no_transfusion_weight,
                             data = diab.subgroup),
  
  # Diabetes Any Outpatient Visits
  diab_any_outpatient_unadj = lm(out_any_outpatient_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                                   biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding,
                                 data = diab.subgroup),
  diab_any_outpatient_adj   = lm(out_any_outpatient_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                                   biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding +
                                   age + no_transfusion_weight,
                                 data = diab.subgroup),
  
  # HTN Medication
  htn_med_unadj = lm(out_htn_med_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                       biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding,
                     data = htn.subgroup),
  htn_med_adj   = lm(out_htn_med_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                       biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding +
                       age + no_transfusion_weight,
                     data = htn.subgroup),
  
  # HTN Postpartum Visits
  htn_postpartum_unadj = lm(out_postpartum_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                              biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding,
                            data = htn.subgroup),
  htn_postpartum_adj   = lm(out_postpartum_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                              biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding +
                              age + no_transfusion_weight,
                            data = htn.subgroup),
  
  # HTN Any Outpatient Visits
  htn_any_outpatient_unadj = lm(out_any_outpatient_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                                  biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding,
                                data = htn.subgroup),
  htn_any_outpatient_adj   = lm(out_any_outpatient_12 ~ biweekly_counter + period_FFCRA + period_Exposure_Unwinding +
                                  biweekly_counter:period_FFCRA + biweekly_counter:period_Exposure_Unwinding +
                                  age + no_transfusion_weight,
                                data = htn.subgroup)
)
    ...
    modify_table_body(~ .x %>%
      dplyr::mutate(
        label = dplyr::case_when(
          ...
          variable == "biweekly_counter:period_Exposure_Unwinding"   ~ "Trend Change, Exposure to Unwinding",
          TRUE ~ label
        )
      )
    )
...
diab_screen_tbl <- tbl_regression(
  ...
  include = c("biweekly_counter", "period_FFCRA", "period_Exposure_Unwinding",
              "biweekly_counter:period_FFCRA", "biweekly_counter:period_Exposure_Unwinding"),
  label = list(
    ...
    `biweekly_counter:period_Exposure_Unwinding` ~ "Trend Change, Exposure to Unwinding"
  ),
...
test_unwinding_dates_simple <- function(data, 
  ...
  fit_model_for_date <- function(unwind_start, data) {
    ...
    # mutually exclusive period indicators
    data_temp <- data %>%
      mutate(
        period_Baseline = if_else(anchor_date >= baseline_start & anchor_date <= baseline_end, 1, 0),
        period_PostUnwinding = if_else(anchor_date >= unwind_start & anchor_date <= data_end_date, 1, 0)
      )
    
    # filter to only baseline and post-unwinding observations
    data_temp <- data_temp %>%
      filter(period_Baseline == 1 | period_PostUnwinding == 1)
    
    # calculate biweekly_counter using baseline_start
    data_temp <- data_temp %>%
      mutate(
         biweekly_counter_new = as.integer(difftime(anchor_date, baseline_start, units = "days") / 14)
      )
    ...
    # Model 1: ITS model with time trends
    model_its <- lm(
      as.formula(paste0(outcome_var, " ~ biweekly_counter_new + period_PostUnwinding + ",
                       "biweekly_counter_new:period_PostUnwinding")),
      data = data_temp
    )
    ...
    b1_its <- coef_its %>% filter(term == "biweekly_counter_new")
    b2_its <- coef_its %>% filter(term == "period_PostUnwinding")
    b3_its <- coef_its %>% filter(term == "biweekly_counter_new:period_PostUnwinding")
...
p1 <- ggplot(cohort_unwinding_tests_simple, aes(x = unwinding_start_date, y = its_level_change)) +
  ...
  labs(
    ...
    subtitle = "Model: Y = B0 + B1*time + B2*Post + B3*time*Post",
...
```{r}
# Function to plot ITS model fit vs raw data
plot_its_model <- function(model, data, outcome_var, title = NULL) {
  
  # Calculate raw means by biweekly period
  raw_means <- data %>%
    group_by(biweekly_counter, period_FFCRA, period_Exposure_Unwinding) %>%
    summarise(
      raw_mean = mean(get(outcome_var), na.rm = TRUE),
      .groups = 'drop'
    ) %>%
    mutate(
      period = case_when(
  ...
  # Create prediction data using actual observed combinations
  pred_data <- data %>%
    select(biweekly_counter, period_FFCRA, period_Exposure_Unwinding) %>%
    distinct() %>%
    mutate(
      period = case_when(
        period_Exposure_Unwinding == 1 ~ "Unwinding",
  ...
  p <- ggplot() +
    ...
    geom_point(data = raw_means, 
               aes(x = biweekly_counter, y = raw_mean, color = period),
               alpha = 0.6, size = 2.5) +
    # Fitted lines
    geom_line(data = pred_data,
              aes(x = biweekly_counter, y = fitted, color = period),
              linewidth = 1.2) +

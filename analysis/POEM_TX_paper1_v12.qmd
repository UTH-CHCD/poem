...
## Figure 1: Biweekly graphs based on anchor date: outpatient utilization (2 panels: diabetes, hypertension) 

```{r}
# Create combined dataset with subgroup labels
visit.plots.subgroups <- bind_rows(
  htn.subgroup %>%
  ...
  mutate(
    ...
    subgroup = factor(subgroup, levels = c("Hypertension", "Diabetes"))
  )
```
...
ggplot() +
  ...
  labs(
    title = "Outpatient Visit Types by Subgroup - 12 Months Post-Delivery",
    x = "Date",
...
diab_screen_tbl <- tbl_regression(
  ...
  modify_header(estimate ~ "**Diabetes Screening**<br>?? (95% CI)") %>%
...
diab_outpatient_tbl <- tbl_regression(
  ...
  modify_column_hide(p.value) %>%
  modify_header(estimate ~ "**Any outpatient utilization**<br>?? (95% CI)") %>%
  fix_interaction_labels()
...
htn_med_tbl <- tbl_regression(
  ...
  modify_column_hide(p.value) %>%
  modify_header(estimate ~ "**Antihypertensive medication**<br>?? (95% CI)") %>%
  fix_interaction_labels()
...
```{r}
# Create combined dataset with subgroup labels - Average number of visits
visit.counts.subgroups <- bind_rows(
  htn.subgroup %>%
    group_by(anchor_date_twoweeks) %>%
    summarise(
...
# Create the plot for average number of visits
ggplot() +
  geom_rect(
    data = period_rects,
  ...
  facet_wrap(~ subgroup, ncol = 2) +
  labs(
    title = "Average Number of Outpatient Visits by Subgroup - 12 Months Post-Delivery",
    x = "Date",
